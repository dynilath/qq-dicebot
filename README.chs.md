...
> DiceBot：``` * dynilath  掷骰: 4d6 = (1 + 1 + 4 + 2) = 8```
...
> DiceBot：``` * dynilath  掷骰: 4d6k3 = (5 + 1 + (1) + 1) = 7```  
...
> DiceBot：``` * dynilath  掷骰: (((4d6 + 3) / 2 + 2d20) + 4 * 1d6) * 150% = ((((2 + 1 + 5 + 4) + 3) / 2 + (11 + 12)) + 4 * (4)) * 150% = 69.75```  

当然，如果你的算式输入格式有问题，多余的部分会被识别成骰子描述。  
> dynilath：```.rs4d6+(((4d6+3)/2+2d20)+4*1d6```

> DiceBot：``` * dynilath +(((4d6+3)/2+2d20)+4*1d6 掷骰: 4d6 = (6 + 3 + 2 + 4) = 15```  
