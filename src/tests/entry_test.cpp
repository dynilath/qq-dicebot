...
TEST_F(entry_test, roll_coc) {
    ...
    ASSERT_TRUE(this->test_call(
        ei, ".cb2p1", std::regex(regex_prefix + " b1" + regex_suffix)));
    ASSERT_TRUE(this->test_call(
        ei, ".cp2b1", std::regex(regex_prefix + " p1" + regex_suffix)));

    std::string regex_prefix_2 = u8" \\* dice test 掷骰: CoC";
    std::regex reg_coc_full_test(
        regex_prefix_2 + " \\(55\\) b1" + regex_suffix
        + " (crit success|extreme success|hard success|success|fail|crit "
          "fail)");
...
TEST_F(entry_test, poker) {
    ...
    std::regex re_draw_two(
        "^ \\* dice 抽出了 ([abcd], [abcd]) \\| 牌堆剩余\\d张");
    std::regex re_draw_three(
        "^ \\* dice 抽出了 ([abcd], [abcd], [abcd]) \\| 牌堆剩余\\d张");
    ...
    ASSERT_NO_THROW(this->base_call(ei, ".pinit tarot"));
    ASSERT_NO_THROW(this->base_call(ei, ".pd 10"));
    ASSERT_NO_THROW(this->base_call(ei, ".pinit major arcana"));
    ASSERT_NO_THROW(this->base_call(ei, ".pd 10"));
