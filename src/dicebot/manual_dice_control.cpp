...
#define MANUALDICE_TABLE_DEFINE "create table " MANUALDICE_TABLE_NAME \
    ...
    bool manual_dice_control::insert_database(manual_kpair manual_dice_key,p_manual manual_dice_target) const{
        ...
        int ret_code_2 = sqlite3_exec(database, ostrs_sql_command.str().c_str(), &database::sqlite3_callback_general, (void*)&database::oper_update, &pchar_err_message);
        #ifdef _DEBUG
    ...
    bool manual_dice_control::exist_database(manual_kpair manual_dice_key) const{
        sqlite3 * database = database::database_manager::get_database();
        ostrs ostrs_sql_command(ostrs::ate);
        ostrs_sql_command << "SELECT count(*) FROM " MANUALDICE_TABLE_NAME " where qqid =" << manual_dice_key.first << " and groupid =" << manual_dice_key.second;
        std::string str_manualdice_read;
        char * pchar_err_message = nullptr;
        int count = 0;
