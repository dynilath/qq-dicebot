...
namespace dicebot::roll{
	...
	dice_roller::dice_roller(std::string & str_single_dice,roll_mode mode)
	{
		this->status = roll_status::UNINITIALIZED;
		if (mode == roll_mode::COC_PB) {
			std::ostringstream ostrs_dice_stream(std::ostringstream::ate);
			...
			try {
				...
				if (i_cal_pb != 0) {
					...
					if (dice_roll2.status != roll_status::FINISHED)
					{ 
					...
					if (dice_roll1.status != roll_status::FINISHED)
					{
					...
					if (dice_roll.status != roll_status::FINISHED)
					{
				...
				try {
					...
					if (i_pos_of_d == std::string::npos) {
						...
						if (i_pos_of_k == std::string::npos) {
							...
							if (dr_diceRoll.status != roll_status::FINISHED)
							{
			...
			this->status = roll_status::GENERAL_ERR;
		}
	}
