...
namespace dicebot{
	...
	bool base_split_dice(const std::string & str_input, std::string & str_output, bool giving_detailed_message) {
		...
		int32_t i_dice_buffer_level_1 = 0;
		double d_dice_buffer_level_1 = 0.0;

		int32_t i_dice_result = 0;
		...
		for (; i_max_unit_alert < MAX_DICE_UNIT_COUNT; i_max_unit_alert++) {
			...
			roll::dice_roller dr_single_dice(str_single_dice);
			if (dr_single_dice.status != roll::roll_status::FINISHED) { is_this_line_output = false; break; }
	...
	bool operation_item::calculate()
	{
		...
		{
		case operation_mode::DICE:{
			roll::dice_roller dice(this->mode_num_like_source);
