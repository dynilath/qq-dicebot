...
{
	this->dice_protocol_map = new std::map<std::string, dice_protocol * >();
}
...
void dice_protocols_manager::register_dice(dice_protocol * protocol)
{
	std::string command = protocol->get_identifier();
	std::map<std::string, dice_protocol * >::iterator  iter = this->dice_protocol_map->find(command);
	if (iter != dice_protocol_map->end()) return;
	dice_protocol_map->insert(std::pair<std::string, dice_protocol * >(command, protocol));
}
...
void dice_protocols_manager::create_command_regex()
{
	...
	ostrs_stream << "^ *\\. *(";
	bool is_first = true;
	std::map<std::string, dice_protocol * >::iterator  iter = this->dice_protocol_map->begin();
